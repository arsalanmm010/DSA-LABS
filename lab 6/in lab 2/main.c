...
int main()
{
    ...
    int src_x;  /// Specifying the start node coordinates in the grid
    int src_y;
    printf("Enter starting coordinates separated by a space:\n");
    scanf("%d %d",&src_x,&src_y);

    int dst_x;  /// Specifying the end node coordinates in the grid
    int dst_y;
    printf("Enter destination coordinates separated by a space: \n");
    ...
    while(! q_isEmpty(&qfront))    /// while the queue is not empty
    {
        ...
        else        /// Add the neighbours with cost added into the que
        {
            ...
            for(int i=0; i<4; i++)      /// Visiting the 4 possible neighbours.
            {
                ...
                if(visited[nx][ny] == 0 && (*((my_grid+nx*num_cols)+ny)==0)) /// if the cell has not been visited yet.
                {

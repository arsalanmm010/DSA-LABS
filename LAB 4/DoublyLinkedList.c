...
void insertNodeAtStart(struct node_d ** head)
{
   ...
   {
        while(ptr->next != *head)     /// iterate though whole list
            ptr = ptr->next;
        new_node->prev = ptr;
        ptr->next = new_node;
        new_node->next = *head;
...
{
	/*** To be completed by the students ***/
	struct node_d * temp = *head, *ptr = *head;
	if(isListEmpty(*head))              /// checking if list is empty
        printf("List is empty\n");
    else if(temp -> next == *head)      /// if list has on;y one node
    {
        *head = NULL;
        free(*head);
        printf("\n only one node was present so, list is now empty");
    }
    else
    {
        ...
        *head = ptr -> next;
        ptr -> prev = temp;
        free(temp);
        printf("\n The list after deletion of first node is!!!\n");
        printList(head);
    }
}
...
*/
void deleteNodeAfter(struct node_d * head, int idx)
{
   /*** To be completed by the students ***/
    int index = 0;
    ...
    if(isListEmpty(head))    /// checking is list is empty
        printf("\n List is empty");
    else
    {
        ptr = temp->next;
        ...
        if((temp->next == head) && (index != idx))      /// We reached the end of the list without index
            printf("\nNode deletion not done. value not found!!\n");
        temp -> next = ptr -> next;
        free(ptr);
    }
...
void printMemMap(struct node_d * head)
{
   /*** To be completed by the students ***/
    ...
    int i = 0;
    printf("No.\t\t address\t\t Data\t\t\t   Next\t\t\t    Prev\n\n");
    do
    {
        printf("%d\t\t %u\t\t %d\t\t\t %u\t\t\t%u\n\n", i, ptr, (ptr->data.age), ptr->next, ptr->prev);
        ptr = ptr->next;
        i++;
    }
    while(ptr != head);

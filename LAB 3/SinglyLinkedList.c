...
int  insertNodeBefore(struct node * head, int idx)
{
    ...
    struct node * temp = head,*ptr;     /// assigning head to temporary pointer
    ...
    while((index != idx) && (temp->next) != NULL)           /// loop will execute as long as both conditions are met
    {
      index ++;
      ptr = temp;
      temp = temp->next;      /// scroll to the end of the list
    ...
    ptr->next = new_node;
    new_node->next = temp;    /// Setting up the pointers for insertion
    return(0);
...
void deleteNodeFromStart(struct node ** head)
{
    ...
    ptr = *head;                /// ptr will be assigned head

    *head = ptr->next;          /// head will be assigned 2nd node

    free(ptr);                  /// 1st node deleted
    }
}
...
int  deleteNodeAfter(struct node * head, int idx)
{
    ...
    else
    {
        ...
        ptr = head;
        preptr = ptr;
        while((preptr -> next != NULL) &&  index != idx)
        {
            index++;
            preptr = ptr;
            ptr = ptr->next;
        }
        if((preptr->next == NULL) && (index != idx))      /// We reached the end of the list without
        {                                               /// reaching the required index
            return(-1);
        }
        temp = ptr->next;
...
int saveListToFile(struct node * head, FILE * fptr)
{
    ...
    if (fptr != NULL)
    {
        ...
        {
            fprintf(fptr, "%s %d %.2f ", head->data.name, head->data.age, head->data.bs);       /// printing the data of employee
        }
    }

...
bool test_palindrome_rec(char * test_word,int index, int size);   /// Complete This
#endif // TASK1
...
int main()
{
        ...
        printf("\nRecursive Output: ");
        if(test_palindrome_rec(my_word,0, strlen(my_word)-2))
            printf("This word is a Palindrom!\n");
        else
            printf("This word is not a Palindrom!\n");
...
bool test_palindrome_rec(char * test_word,int index, int size)
{
	...
	if (*(test_word + index) != *(test_word + size))
		return false;

	return test_palindrome_rec(test_word, index + 1, size -1);
}
#endif // TASK1
